...
*   **Integração Nativa com Apple Wallet & Google Wallet:** O cliente final pode salvar passes dinâmicos e estéticos diretamente nas carteiras virtuais do celular. Suporta o modelo **Event Ticket** (ingresso do agendamento com data/hora e local) ao concluir agendamento no `PortalPublicBooking.tsx`, e modelo **Loyalty Pass** (cartão de carimbos gamificado com progresso de visitas) na tela `ConfirmarPresenca.tsx`, com suporte a cores customizadas (`walletCardColor` e `walletTextColor`) definidas pelo estabelecimento.
