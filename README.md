...
*   **Fechamento do Mês (PDF Executivo A4):** Relatório de fechamento estético executivo otimizado para salvamento em PDF ou impressão física no formato padrão A4, ocultando os elements do portal na impressão (`print:hidden`).
*   **Cardápio Digital Público & Delivery:** Rota pública sem autenticação (`/cardapio/:orgId`) onde os clientes podem consultar o catálogo de produtos e lanches, selecionar adicionais, cadastrar dados de entrega, e enviar o pedido diretamente via WhatsApp. A ação deduz do estoque central, registra no Financeiro e cadastra o cliente no CRM de forma integrada.
*   **Mesa de Pedidos Delivery da Cozinha (`PortalOrders.tsx`):** Aba de monitoramento operacional em tempo real no painel administrativo com notificações sonoras ativas (via Web Audio API) para gerenciar e despachar os pedidos que chegam do cardápio digital.
